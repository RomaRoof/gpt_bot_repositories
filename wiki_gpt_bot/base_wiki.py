...
CACHE_FILE = "wiki_cache.txt"
CATEGORY_TITLE = "Категория:Педиатрия"
WIKI_SITE = "ru.wikipedia.org"


# Собираем заголовки статей
    ...
    for cm in category.members():  # Перебираем вложенные объекты категории
        if type(cm) == mwclient.page.Page:  # Если объект является страницей
            titles.add(cm.name)  # в хранилище заголовков добавляем имя страницы
                        ...
                        mwclient.listing.Category) and max_depth > 0:  # Если объект является категорией и глубина вложения не достигла максимальной
            deeper_titles = titles_from_category(cm,
                                                 max_depth=max_depth - 1)  # вызываем рекурсивно функцию для подкатегории
            titles.update(deeper_titles)  # добавление в множество элементов из другого множества
    return titles


def initialize_wiki_base():
    if not os.path.exists(CACHE_FILE):
        print("Выполняется base_wiki.py...")
        language = "ru"
        # Установим русский  язык, значит будем выполнять поиск в русско-язычном секторе Википедии (https://ru.wikipedia.org/)
        wikipedia.set_lang(language)

        # Ищем информацию о ...
        result = wikipedia.search("Педиатрия")
        print(result)

        # Инициализация объекта MediaWiki
        # WIKI_SITE ссылается на ру часть Википедии
        ...
        # os.environ["OPENAI_API_KEY"] = input(getpass.getpass("Введите OpenAI API Key:"))

        # Функция отправки chatGPT строки для ее токенизации (вычисления эмбедингов)
        def get_embedding(text, model="text-embedding-ada-002"):
            return client.embeddings.create(input=[text], model=model).data[0].embedding

        df = pd.DataFrame({"text": wikipedia_strings[:1000]})

        df['embedding'] = df.text.apply(lambda x: get_embedding(x, model='text-embedding-ada-002'))

        SAVE_PATH = "./medicine.csv"
        # Сохранение результата
        df.to_csv(SAVE_PATH, index=False)
        with open(CACHE_FILE, 'w') as cache_file:
            cache_file.write("Код выполнен")
    else:
        print("wiki_base.py уже был выполнен, пропуск.")
